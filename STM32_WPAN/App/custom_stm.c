...
/* Private variables ---------------------------------------------------------*/
uint16_t SizeLed_C = 512;
uint16_t SizeSwitch_C = 512;
...
/* USER CODE BEGIN PF */
/**
 * @brief  Characteristic extended update
 * @param  CharOpcode: Opcode of the characteristic
 * @param  Connection_Handle: Current BLE connection handle
 * @param  Length: Length of payload
 * @param  Payload: Payload to update characteristic
 * 
 */
tBleStatus Custom_STM_App_Update_Char_Ext(Custom_STM_Char_Opcode_t CharOpcode, uint16_t ConnectionHandle, uint16_t Length, uint8_t *pPayload) 
{
  ...
  switch(CharOpcode)
  {
    case CUSTOM_STM_SWITCH_C:
      ...
      result = aci_gatt_update_char_value_ext(ConnectionHandle,
      ...
      {
       APP_DBG_MSG("  Fail   : aci_gatt_update_char_value_ext SWITCH_C command, result : 0x%x\r\n", result);
       return result;
      }
      else
      {
       APP_DBG_MSG("  Success: aci_gatt_update_char_value_ext SWITCH_C command\r\n");
      }
        ...
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          else if (attribute_modified->Attr_Handle == (CustomContext.CustomLed_CHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            ...
            */
            ...
            Notification.Offset=attribute_modified->Offset;
            Custom_STM_App_Notification(&Notification);
