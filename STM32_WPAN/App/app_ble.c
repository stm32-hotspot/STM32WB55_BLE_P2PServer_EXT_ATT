...
uint8_t a_AdvData[25] =
{
  ...
  7, AD_TYPE_COMPLETE_LOCAL_NAME, 'P', '2', 'P', 'S', 'R', 'V',  /* Complete name */
...
void APP_BLE_Init(void)
{
  ...
  UTIL_SEQ_RegTask( 1<<CFG_TASK_SW3_BUTTON_PUSHED_ID, UTIL_SEQ_RFU, Exchange_MTU);
    ...
    case HCI_LE_META_EVT_CODE:
    {
      ...
      switch (p_meta_evt->subevent)
      {
        ...
        case HCI_LE_CONNECTION_COMPLETE_SUBEVT_CODE:
        {
          ...
          ret = hci_le_set_data_length(BleApplicationContext.BleApplicationContext_legacy.connectionHandle,251,2120);
          if (ret != BLE_STATUS_SUCCESS)
          {
...
static void Exchange_MTU(void)
{
  ...
  {
    APP_DBG_MSG("GATT Exchange - ATT MTU : Success\n");
  }
  else
  {
    APP_DBG_MSG("GATT Exchange - ATT MTU : Failed (%x)\n", status);
  }
}
